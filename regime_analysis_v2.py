...
REGIMES = ("CRASH", "HIGH_VOL", "BEAR", "BULL", "SIDEWAYS", "RECOVERY", "OTHER")
...
def classify_regimes(data: pd.DataFrame) -> pd.Series:
    """Classify each bar into regimes using the multi-factor rules."""
    close = data["close"]
    ...
    valid = ~(sma_50.isna() | sma_200.isna() | atr_50.isna() | atr_14.isna() | from_high_50.isna() | from_low_50.isna())

    crash = valid & (from_high_50 < -0.15)
    high_vol = valid & (atr_14 > atr_50 * 1.5)
